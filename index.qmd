...
- Why might high altitude affect baseball performance?<br>  

*I'm not familiar with baseball but I think with high altitude the air will be thinner so when the ball in airborn it will has less air resistance and therefore fly further or faster.*<br>

- Is Coors Field affected by high altitude?<br>

*To be honest I don't even know where this place is but according to AI it is at 5,280ft which is considered high altitude. So I would imagine COL is somewhat affected by high altitude.*<br>
:::
...
- How many games are in the dataset? <br>

*There are 2429 games in the 2021 dataset and 2430 games in the 2010 dataset.*<br>

- Why do we aggregate individual games into stadium averages before we start the visualization process?<br>

*We aggregate individual games into statdium averages because we are trying to tell if this one stadium(COL) is the most run-friendely one. If we don't aggregate there will be too many data points fot one single stadium which is hard to tell anything.*<br>
:::
...
- Why is a horizontal bar chart better than a scatter plot for this data?<br>

*I think scatter plot is either too many data points without aggregation or not as clear as bar chart because data points are small dots instead of bars that are easy to compare lengths ie.values that we care about. The horizontal bar chart shows one bar per stadium, length = average runs, so readers can compare magnitudes and rank stadiums at a glance.*<br>

- When would you choose a vertical bar chart over horizontal?<br>

*I think based on the example in the code above, I would choose vertical when I have only few categories and short category names that fit under the axis without rotation. I would choose horizontal when O have many categories (like 30 stadiums) or longer labels—horizontal bars give more room for y-axis labels and avoid cramped, rotated text, which can help improves readability.*<br>

**Question 2: Aesthetic Mappings**
- What does the color highlighting accomplish in Approach 4?<br>

*Approach 4 uses color to clearlyshow and draw viewers' attention to which stadium has the most average runs. Without highlighting, every bar looks equally important.*<br>

- How does position (x/y) compare to color for encoding data?<br>

*I think most situation color is better used as a secondary channel like highlighting or emphasis, grouping or for precise numeric values. So in Stage 3 we use position for the core data (which stadium, how many runs) and color to emphasize our subject of interest (Colorado) within that structure. Position (x/y) is most like a primary channel for magnitude and category which bar length and bar position carry the main quantitative and categorical information that no matter there is color or not, we can still tell which stadium has the most average runs.*<br>
...
#colorado_index = avgDF_2010_sorted
#[avgDF_2010_sorted.home == "COL"].index[0]
#colorado_runs = avgDF_2010_sorted
#[avgDF_2010_sorted.home == "COL"]
#[totalRuns"].iloc[0]
#ax.annotate(f"COL: {colorado_runs:.2f} 
#runs/game", 
           # xy=(colorado_runs, 
           # colorado_index), 
            #xytext=(colorado_runs + 0.5, 
            #colorado_index),
# Add annotation for Colorado (use bar position, not DataFrame index)
...
ax.annotate(f"COL: {colorado_runs:.2f} runs/game",
            ...
            xytext=(colorado_runs + 0.4, colorado_bar_pos),
...
- What elements make this visualization suitable for a business presentation?<br>

*The elements make this visualization suitable for a business presentation because it has many elements that we talked about in class specifically Stage 4 (Formatting for Your Audience) It has bold title states the giving the opinion.The chart uses the right aggregated metric with the horizontal bars clearly show the different between stadiums.Alsi the chart has clear axis labels and units, a legend, unique color for the highest value bar to draw attention to it. It also attampted to have the annotation to point at the highest value bar to show the exact value but it is not pointing at the correct bar.*<br>

- Is the annotation on the visualization helpful?  Can you fix its placement?<br>

*It could be helpful but the locaation is wrong. It is not pointing at the correct bar which is the top one.*<br>
...
# Two-facet figure: 2010 and 2021
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))

def plot_year(ax, runs_col, year_label):
    """Plot one year: bars, highlight COL, annotate COL and highest bar."""
    df = comparison_data.sort_values(runs_col, ascending=True)
    homes = df['home'].values
    runs = df[runs_col].values
    colors = ["darkorchid" if h == "COL" else "lightgrey" for h in homes]
    ax.barh(homes, runs, color=colors)
    ax.set_xlabel("Average Runs Per Game", fontsize=11)
    ax.set_ylabel("Stadium (Home Team)", fontsize=11)
    ax.set_xlim(0, max(runs) * 1.15)
    ax.tick_params(axis='y', labelsize=9)
    ax.grid(True, alpha=0.3)

    # Which stadium has the most runs this year
    idx_max = df[runs_col].idxmax()
    top_row = df.loc[idx_max]
    top_stadium = top_row['home']
    top_runs = top_row[runs_col]
    ax.set_title(f"{year_label}: {top_stadium} has the most runs ({top_runs:.2f}/game)",
                 fontsize=12, fontweight='bold', pad=10)

    # Annotation: COL value (bar position is 0-based index in sorted list)
    col_pos = np.where(homes == "COL")[0][0]
    col_runs = float(df.loc[df['home'] == "COL", runs_col].iloc[0])
    ax.annotate(f"COL: {col_runs:.2f}",
                xy=(col_runs, col_pos),
                xytext=(col_runs + 0.35, col_pos),
                arrowprops=dict(arrowstyle='->', color='darkorchid', lw=1.5),
                fontsize=9, fontweight='bold', color='darkorchid')

    # Annotation: highest bar value (only if different from COL to avoid duplicate)
    max_pos = np.where(homes == top_stadium)[0][0]
    if top_stadium != "COL":
        ax.annotate(f"Max: {top_stadium} {top_runs:.2f}",
                    xy=(top_runs, max_pos),
                    xytext=(top_runs + 0.35, max_pos),
                    arrowprops=dict(arrowstyle='->', color='black', lw=1.5),
                    fontsize=9, fontweight='bold')

plot_year(ax1, 'runs_2010', '2010')
plot_year(ax2, 'runs_2021', '2021')
...
ax1.legend(handles=[colorado_patch, other_patch], loc='lower right', frameon=True)

fig.suptitle("Stadium run-friendliness: which stadium has the most runs?", fontsize=14, fontweight='bold', y=1.02)
plt.tight_layout()
plt.show()
```
...
- Create a two-facet visualization that shows the total runs for 2010 and 2021 for each stadium in a single figure.  Highlight Colorado in the visualization.<br>

**Question 2: Explanation of the Visualization**
- *The visualization compares average runs per game by stadium in two side-by-side panels (2010 and 2021). Each panel is a horizontal bar chart with stadiums on the y-axis and average runs on the x-axis. Colorado (COL) is highlighted in purple so we can track Coors Field across both years; other stadiums are gray. Annotations label COL’s value in each year and, when different, the stadium with the highest runs. The panel titles state which stadium has the most runs in that year (and the value). From this we can see whether COL was the most run-friendly ballpark in 2010, in 2021, or in both, and how the gap between COL and the rest of the league changed over time.*<br>

- Does AI come to the right conclusion?  If not, why not?<br>

*Based on the visualization I would say the AI came to the right conclusion. COL is the most run-friendly stadium in both 2010 but not in 2021. BOS had the most runs in 2021 and COL was in the third place in 2021.*<br>
:::
